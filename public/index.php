	...
	<div class="container-fluid text-white h-100">
		<div class="d-lg-flex align-items-center justify-content-between text-center pl-lg-5">
			...
			<div class="col align-self-bottom align-items-right text-right h-max-380 position-relative z-index-1">
				<!-- <img data-aos="fade-up" src="<?php // echo url_for('../public/assets/img/f1car.png'); 
...
<!-- Start Favourite Races Section -->
<?php 
if (is_logged_in()) {
	$isFav = find_favs();
}
if (is_logged_in() && !empty($isFav)) { 
	
?>
	<div class="container pt-5 pb-5" data-aos="fade">
		<div class="row">
			<h2><strong>My Favourite Races</strong>
				<a class="btn btn-outline-primary ml-3 mb-1 btn-sm " href="<?php echo url_for('/account.php'); ?>"> VIEW ALL</a>
			</h2>
		</div>
		<div class="row gap-y">
			<?php
				$favourite_information = find_favourite_information_by_user();
				while ($favs = mysqli_fetch_assoc($favourite_information)) {

					$my_races_set = find_race_by_raceId_with_limit($favs['raceId']);

					while ($my_races = mysqli_fetch_assoc($my_races_set)) {
						$my_circuit_set = find_race_by_circuitId($my_races['circuitId']);
						while ($my_circuits = mysqli_fetch_assoc($my_circuit_set)) {

							?>
						<div class="col-md-6 col-xl-4">
							<div class="card shadow-sm border-0" data-aos="fade">
								<img class="card-img-top" src="../public/assets/img/recent-race-img/<?php echo $my_circuits['country']; ?>.jpg" alt="Card image cap">
								<div class="card-body">
									<h5 class="card-title text-secondary"><a target="_blank" href="<?php echo $my_races['url']; ?>"><?php echo $my_races['name']; ?></a></h5>

									<div class="row ml-0">
									<i class="fas fa-calendar-day mr-2 text-primary"></i>
									<h6>										<?php
										echo date_format(date_create(h($my_races['date'])), "M jS, Y");
										?>
									</h6>
									</div>
									...
									<h6>
									<a class="text-dark" data-toggle="tooltip" data-placement="right" data-original-title="<?php echo h($my_circuits['lat']) . ", " . h($my_circuits['lng']); ?>" class="text-dark" target="_blank" href="http://google.com/maps/place/ <?php echo h($my_circuits['lat']) . ', ' . h($my_circuits['lng'])  . "\">"; ?>
                                    <?php
											echo h($my_circuits['location']);
											// When specific location doesn't exist, the comma separator is not displayed
											if (!is_blank(h($my_circuits['location']))) {
												echo ', ';
											}
											?> 
                                    <?php

											echo h($my_circuits['country']);
											?>
                                    </a>
									</h6>
									</div>	

									<?php if (!is_blank($my_races['time'])) {  ?>
                                <div class=" row ml-0">
											<i class="fas fa-clock mr-2 text-primary"></i>
											<h6><?php echo date_format(date_create(h($my_races['time'])), "g:i A");  ?></h6>
								</div>
							<?php } ?>

							<div>
							<a href="<?php echo url_for('/race-details.php?raceId=') . $my_races['raceId'] ?>" class='btn mt-2 btn-sm btn-primary ${btnBlock} text-white'>View Details</a>
							</div>
	...
	<!-- End Favourite Races Section -->


	<!-- Start Recent Races -->
	<div class="container pb-5 <?php if (!is_logged_in() || (is_logged_in() && empty($isFav))) { echo "pt-5"; } ?>" data-aos="fade" id="upcoming">
		<div class="row">
						...
						<div class="card shadow-sm border-0" data-aos="<?php echo $fade[$loop_fade_array]; ?>">
							...
							<div class="card-body">
								<h5 class="card-title text-secondary">
									<a target="_blank" href="<?php echo $recent_races['url']; ?>"><?php echo $recent_races['name']; ?></a>
								</h5>
								<div class="row ml-0">
									<i class="fas fa-calendar-day mr-2 text-primary"></i>
									<h6>
